...
def _target_check(target: dict[str, str], args: argparse.Namespace) -> tuple[Check, dict[str, Any]]:
    ...
    row["balance_eth"] = f"{balance / 10**18:.8f}"
    if balance <= 0:
        check = Check(
            f"funding_{label}",
            "red",
        ...
        row["remediation"] = check.remediation
        return check, row
    check = Check(f"funding_{label}", "green", f"{label} {address} has {balance / 10**18:.8f} ETH on Base Sepolia.")
    row["status"] = check.status
    return check, row
...
def build_report(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for target in raw_targets:
        check, row = _target_check(target, args)
        checks.append(check)
...
def _print_text(report: dict[str, Any]) -> None:
    ...
    for target in report.get("funding_targets") or []:
        ...
        print(f"- [{target.get('status')}] {target.get('label')}: {target.get('address') or 'missing'} ({balance_text})")
