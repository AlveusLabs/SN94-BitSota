...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Headless Pool lease driver using the sidecar job queue.")
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    coord = PoolLeaseCoordinator(
        ...
        request_interval_s=max(0.2, float(args.request_interval_s)),
    )
