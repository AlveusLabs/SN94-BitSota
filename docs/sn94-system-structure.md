...
flowchart LR
  ...
  Backend -->|competition scores| Pool[Pool service]
...
Validators run their own validation/verifier processes with their own keys.
...
Each validator independently replays the submission and posts observed metrics.
The backend combines validator results, usually using accepted validator
metrics and median numeric values where multiple validators report.
...
- `SN94-BitSota` checkout at the current validator branch/fix level;
