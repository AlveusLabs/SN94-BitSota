...
class Task(ABC):
    ...
    def _predict_after_training(
        ...
        *,
        ...
        if use_shared_memory:
            predictions = executor.execute_shared_memory_numba(
                X_train,
                y_train,
                X_val,
                epochs=epochs,
            ...
            executor.record_shared_fallback()
            ...
            for _ in range(epochs):
                for idx in range(train_len):
                    train_input[0] = X_train[idx]
                    train_labels[0] = y_train[idx]
            ...
            val_input = np.zeros((batch_size, input_dim), dtype=np.float32)
            for idx in range(val_len):
                val_input[0] = X_val[idx]
    ...
    def evaluate_algorithm(
        ...
        try:
            ...
            predictions = self._predict_after_training(
                algo_array,
                self.X_train,
                self.y_train,
                self.X_val,
